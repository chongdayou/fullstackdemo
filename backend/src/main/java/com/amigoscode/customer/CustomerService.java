...
public class CustomerService {
    ...
    public void deleteCustomerById(Integer customerId) {
        checkIfCustomerExistsByIdOrThrow(customerId);

        customerDao.deleteCustomerById(customerId);
    }

    private void checkIfCustomerExistsByIdOrThrow(Integer customerId) {
    ...
    public void uploadCustomerProfileImage(Integer customerId, MultipartFile file) {
        ...
        customerDao.updateCustomerPorfileImageId(profileImageId, customerId);
    }

    public byte[] getCustomerProfileImage(Integer customerId) {
        var customer = customerDao.selectCustomerById(customerId)
                .map(customerDTOMapper) // exclude information about customer that is not needed here
                .orElseThrow(() -> new ResourceNotFoundException(
                        "customer with id [%s] not found".formatted(customerId)
                ));
        ...
        byte[] profileImage = s3Service.getObject(
                buckets.getCustomer(),
                "profile-image/%s/%s".formatted(customerId, customer.profileImageId())
        );
